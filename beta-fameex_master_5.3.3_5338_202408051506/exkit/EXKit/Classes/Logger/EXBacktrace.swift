...
class EXBacktrace {
    ///
    static let regex = try? NSRegularExpression(pattern: #"(?<index>\d\s+)(?<module>\w+)\s+(?<address>0x[\d|a-f]+)\s+(?<function>[\$\w]+)\s+\+\s+(?<offset>\d+)"#)
    ///
    class func parser(_ callStackSymbols:[String]) -> [String] {
        return callStackSymbols.enumerated().map {(idx,element) in
            if let result = regex?.firstMatch(in: element, range: NSRange(location: 0, length: element.count)),
               let indexRange = Range(result.range(withName: "index"), in: element),
               let functionRange = Range(result.range(withName: "function"), in: element),
               let simplifiedFunction = try? EXBacktrace.parseMangledSwiftSymbol(String(element[functionRange])).description {
                /// replace reversed
                var string = element
                /// update function
                string.replaceSubrange(functionRange, with: simplifiedFunction)
                /// update stack depth
                string.replaceSubrange(indexRange, with: String(format: "%-4ld", idx))
                return string
            }
            return element
        }.filter { !$0.isEmpty }
...
extension EXBacktrace {
    ...
    static func parseMangledSwiftSymbol(_ mangled: String, isType: Bool = false) throws -> SwiftSymbol {
    ...
    /// - Returns: the successfully parsed result
    /// - Throws: a SwiftSymbolParseError error that contains parse position when the error occurred.
    static func parseMangledSwiftSymbol<C: Collection>(_ mangled: C, isType: Bool = false) throws -> SwiftSymbol where C.Iterator.Element == UnicodeScalar {
...
extension EXBacktrace {
    ...
    struct Demangler<C> where C: Collection, C.Iterator.Element == UnicodeScalar {
        ...
        var failure: Error { scanner.unexpectedError() }
        ...
        mutating func demangleType() throws -> SwiftSymbol {
            reset()
            try parseAndPushNames()
            return pop() ?? SwiftSymbol(kind: .suffix, children: [], contents: .name(String(String.UnicodeScalarView(scanner.scalars))))
        }
        ...
        mutating func demangleOperator() throws -> SwiftSymbol {
            switch try scanner.readScalar() {
                ...
                case "s": return SwiftSymbol(kind: .module, contents: .name(SwiftSymbol.stdlibName))
        ...
        mutating func demangleMultiSubstitutions() throws -> SwiftSymbol {
            ...
            while true {
                ...
                if c.ex_backtrace_isLower {
                    ...
                    return try require(substitutions.ex_backtrace_at(idx))
                } else {
                    try scanner.backtrack()
        ...
        mutating func pushMultiSubstitutions(repeatCount: UInt32, index: Int) throws -> SwiftSymbol {
            try require(repeatCount <= SwiftSymbol.maxRepeatCount)
            let nd = try require(substitutions.ex_backtrace_at(index))
            if repeatCount > 1 {
        ...
        mutating func getLabel(params: inout SwiftSymbol, idx: Int) throws -> SwiftSymbol {
            if isOldFunctionTypeMangling {
                let param = try require(params.children.ex_backtrace_at(idx))
        ...
        mutating func popFunctionParamLabels(type: SwiftSymbol) throws -> (type: SwiftSymbol, labelList: SwiftSymbol)? {
            ...
            if topFuncType.kind == .dependentGenericType {
                funcType = try require(topFuncType.children.ex_backtrace_at(1)?.children.first)
            } else {
            ...
            if parameterType.kind == .throwsAnnotation {
                parameterType = try require(funcType.children.ex_backtrace_at(1))
            }
            ...
            if type.children.first?.kind == .dependentGenericType {
                let modifiedDependentType = try require(topFuncType.children.ex_backtrace_at(1)).changeChild(modifiedFuncType, atIndex: 0)
        ...
        mutating func demangleStandardSubstitution() throws -> SwiftSymbol {
            switch try scanner.readScalar() {
                case "o": return SwiftSymbol(kind: .module, contents: .name(SwiftSymbol.objcModule))
                case "C": return SwiftSymbol(kind: .module, contents: .name(SwiftSymbol.cModule))
                case "g":
        ...
        mutating func demangleIdentifier() throws -> SwiftSymbol {
            ...
            let c = try scanner.read(where: { $0.ex_backtrace_isDigit })
            if c == "0" {
            ...
            var identifier = ""
            repeat {
                while hasWordSubs && scanner.peek()?.ex_backtrace_isLetter == true {
                    let c = try scanner.readScalar()
                    var wordIndex = 0
                    if c.ex_backtrace_isLower {
                    ...
                    try require(wordIndex < SwiftSymbol.maxNumWords)
                    identifier.append(try require(words.ex_backtrace_at(wordIndex)))
                }
                ...
                if isPunycoded {
                    identifier.append(EXBacktrace.decodeSwiftPunycode(text))
                } else {
                    identifier.append(text)
                    var word: String?
                    for c in text.unicodeScalars {
                        if word == nil, !c.ex_backtrace_isDigit && c != "_" && words.count < SwiftSymbol.maxNumWords {
                            word = "\(c)"
                        } else if let w = word {
                            if (c == "_") || (w.unicodeScalars.last?.ex_backtrace_isUpper == false && c.ex_backtrace_isUpper) {
                                if w.unicodeScalars.count >= 2 {
                                    words.append(w)
                                }
                                if !c.ex_backtrace_isDigit && c != "_" && words.count < SwiftSymbol.maxNumWords {
                                    word = "\(c)"
                                } else {
                                    word = nil
                                }
        ...
        mutating func demangleOperatorIdentifier() throws -> SwiftSymbol {
            ...
            for c in (try require(ident.text)).unicodeScalars {
                if !c.isASCII {
                    ...
                    try require(c.ex_backtrace_isLower)
                    let o = try require(opCharTable.ex_backtrace_at(Int(c.value - UnicodeScalar("a").value)))
                    try require(o != " ")
        ...
        mutating func demangleBoundGenericArgs(nominal: SwiftSymbol, array: [SwiftSymbol], index: Int) throws -> SwiftSymbol {
            ...
            let args = try require(array.ex_backtrace_at(index))
            ...
            if index + 1 < array.count {
                ...
                if context.kind == .extension {
                    let p = try demangleBoundGenericArgs(nominal: try require(context.children.ex_backtrace_at(1)), array: array, index: index + 1)
                    boundParent = SwiftSymbol(kind: .extension, children: [try require(context.children.first), p])
                    if let thirdChild = context.children.ex_backtrace_at(2) {
                        boundParent.children.append(thirdChild)
                    }
                } else {
                    boundParent = try demangleBoundGenericArgs(nominal: context, array: array, index: index + 1)
                }
                n = SwiftSymbol(kind: nominal.kind, children: [boundParent, try require(nominal.children.ex_backtrace_at(1))])
            } else {
                n = nominal
            }
        ...
        mutating func addFuncSpecParamNumber(param: inout SwiftSymbol, kind: FunctionSigSpecializationParamKind) throws {
            ...
            let str = scanner.readWhile { $0.ex_backtrace_isDigit }
        ...
        mutating func demangleObjCTypeName() throws -> SwiftSymbol {
            ...
            if scanner.conditional(scalar: "C") {
                ...
                if scanner.conditional(scalar: "s") {
                    module = SwiftSymbol(kind: .module, contents: .name(SwiftSymbol.stdlibName))
                } else {
        ...
        mutating func demangleSwift3ProtocolName() throws -> SwiftSymbol {
            ...
            if scanner.conditional(scalar: "S") {
                ...
                let stdlib = SwiftSymbol(kind: .module, contents: .name(SwiftSymbol.stdlibName))
            ...
            if wrapEntity {
                ...
                switch name?.kind {
                    ...
                    case .some(.privateDeclName):
                        if let n = name, let first = n.children.ex_backtrace_at(0), let second = n.children.ex_backtrace_at(1), second.text == "subscript" {
                            isSubscript = true
        ...
        mutating func demangleSwift3Context() throws -> SwiftSymbol {
            switch try scanner.readScalar() {
                ...
                case "S": return try demangleSwift3SubstitutionIndex()
                case "s": return SwiftSymbol(kind: .module, children: [], contents: .name(SwiftSymbol.stdlibName))
        ...
        func swiftStdLibType(_ kind: SwiftSymbol.Kind, named: String) -> SwiftSymbol {
            return SwiftSymbol(kind: kind, children: [SwiftSymbol(kind: .module, contents: .name(SwiftSymbol.stdlibName)), SwiftSymbol(kind: .identifier, contents: .name(named))])
        }
        ...
        mutating func demangleSwift3GenericParamIndex() throws -> SwiftSymbol {
            ...
            return SwiftSymbol(kind: .dependentGenericParamType, children: [SwiftSymbol(kind: .index, contents: .index(depth)), SwiftSymbol(kind: .index, contents: .index(index))], contents: .name(EXBacktrace.archetypeName(index, depth)))
        }
        ...
        mutating func demangleSwift3Type() throws -> SwiftSymbol {
            ...
            switch try scanner.readScalar() {
                ...
                case "x": type = SwiftSymbol(kind: .dependentGenericParamType, children: [SwiftSymbol(kind: .index, contents: .index(0)), SwiftSymbol(kind: .index, contents: .index(0))], contents: .name(EXBacktrace.archetypeName(0, 0)))
        ...
        mutating func demangleSwift3ArchetypeType() throws -> SwiftSymbol {
            switch try scanner.readScalar() {
                ...
                case "S":
                    ...
                    return result
                case "s":
                    let root = SwiftSymbol(kind: .module, contents: .name(SwiftSymbol.stdlibName))
        ...
        mutating func demangleSwift3Identifier(kind: SwiftSymbol.Kind? = nil) throws -> SwiftSymbol {
            ...
            if isPunycode {
                identifier = EXBacktrace.decodeSwiftPunycode(identifier)
            }
            if isOperator {
...
extension EXBacktrace {
    struct ScalarScanner<C: Collection> where C.Iterator.Element == UnicodeScalar {
        ...
        mutating func conditionalInt() -> Int? {
            ...
            var i = index
            var c = 0
            while i != scalars.endIndex && scalars[i].ex_backtrace_isDigit {
...
extension EXBacktrace {
    ///
    struct SwiftSymbol:CustomStringConvertible {
        static let stdlibName = "Swift"
        static let objcModule = "__C"
        static let cModule = "__C_Synthesized"
        static let lldbExpressionsModuleNamePrefix = "__lldb_expr_"
        ...
        init(swiftStdlibTypeKind: Kind, name: String) {
            self.init(kind: .type, children: [SwiftSymbol(kind: swiftStdlibTypeKind, children: [
                SwiftSymbol(kind: .module, contents: .name(SwiftSymbol.stdlibName)),
        ...
        func print(using options: Printer.Options = .default) -> String {
        ...
        var isSwiftModule: Bool {
            return kind == .module && text == SwiftSymbol.stdlibName
        }
...
extension EXBacktrace {
    ...
    struct Printer {
        ...
        struct Options: OptionSet {
            ...
            ///
            static let synthesizeSugarOnTypes         = Options(rawValue: 1 << 0)
            static let displayDebuggerGeneratedModule = Options(rawValue: 1 << 1)
            static let qualifyEntities                = Options(rawValue: 1 << 2)
            static let displayExtensionContexts       = Options(rawValue: 1 << 3)
            static let displayUnmangledSuffix         = Options(rawValue: 1 << 4)
            static let displayModuleNames             = Options(rawValue: 1 << 5)
            static let displayGenericSpecializations  = Options(rawValue: 1 << 6)
            static let displayProtocolConformances    = Options(rawValue: 1 << 5)
            static let displayWhereClauses            = Options(rawValue: 1 << 8)
            static let displayEntityTypes             = Options(rawValue: 1 << 9)
            
            static let shortenPartialApply            = Options(rawValue: 1 << 10)
            static let shortenThunk                   = Options(rawValue: 1 << 11)
            static let shortenValueWitness            = Options(rawValue: 1 << 12)
            static let shortenArchetype               = Options(rawValue: 1 << 13)
            
            static let showPrivateDiscriminators      = Options(rawValue: 1 << 14)
            static let showFunctionArgumentTypes      = Options(rawValue: 1 << 15)
            ///
            static let `default`: Options  = [.synthesizeSugarOnTypes, .qualifyEntities, .shortenPartialApply, .shortenThunk, .shortenValueWitness, .shortenArchetype]
            static let medium   : Options  = [.displayDebuggerGeneratedModule, .qualifyEntities, .displayExtensionContexts, .displayUnmangledSuffix, .displayModuleNames, .displayGenericSpecializations, .displayProtocolConformances, .displayWhereClauses, .displayEntityTypes, .showPrivateDiscriminators, .showFunctionArgumentTypes]
            static let full     : Options  = .init(rawValue: .max)
        }
        ...
        mutating func printFirstChild(_ ofName: SwiftSymbol, prefix: String? = nil, suffix: String? = nil, asPrefixContext: Bool = false) {
            _ = printOptional(ofName.children.ex_backtrace_at(0), prefix: prefix, suffix: suffix)
        }
        ...
        mutating func printName(_ name: SwiftSymbol, asPrefixContext: Bool = false) -> SwiftSymbol? {
            switch name.kind {
                ...
                case .outlinedAssignWithCopy: name.children.ex_backtrace_at(0)?.index.map { target.write("outlined variable #\($0) of ") }
                ...
                case .anonymousContext:
                    if options.contains(.qualifyEntities) && options.contains(.displayExtensionContexts) {
                        _ = printOptional(name.children.ex_backtrace_at(1))
                        target.write(".(unknown context at " + (name.children.first?.text ?? "") + ")")
                        if let second = name.children.ex_backtrace_at(2), !second.children.isEmpty {
                            target.write("<")
                            _ = printName(second)
                            target.write(">")
                        }
                ...
                case .extension:
                    ...
                    printSequence(name.children.ex_backtrace_slice(1, 3))
                ...
                case .explicitClosure: return printEntity(name, asPrefixContext: asPrefixContext, typePrinting: options.contains(.showFunctionArgumentTypes) ? .functionStyle : .noType, hasName: false, extraName: "closure #", extraIndex: (name.children.ex_backtrace_at(1)?.index ?? 0) + 1)
                case .implicitClosure: return printEntity(name, asPrefixContext: asPrefixContext, typePrinting: options.contains(.showFunctionArgumentTypes) ? .functionStyle : .noType, hasName: false, extraName: "implicit closure #", extraIndex: (name.children.ex_backtrace_at(1)?.index ?? 0) + 1)
                ...
                case .defaultArgumentInitializer: return printEntity(name, asPrefixContext: asPrefixContext, typePrinting: .noType, hasName: false, extraName: "default argument \(name.children.ex_backtrace_at(1)?.index ?? 0)")
                ...
                case .localDeclName: _ = printOptional(name.children.ex_backtrace_at(1), suffix: " #\((name.children.ex_backtrace_at(0)?.index ?? 0) + 1)")
                case .privateDeclName:
                    _ = printOptional(name.children.ex_backtrace_at(1), prefix: options.contains(.showPrivateDiscriminators) ? "(" : nil)
                    target.write(options.contains(.showPrivateDiscriminators) ? "\(name.children.count > 1 ? " " : "(")in \(name.children.ex_backtrace_at(0)?.text ?? ""))" : "")
                ...
                case .genericSpecializationParam:
                    ...
                    _ = printOptional(name.children.ex_backtrace_at(1), prefix: " with ")
                    name.children.ex_backtrace_slice(2, name.children.endIndex).forEach {
                        target.write(" and ")
                        _ = printName($0)
                    }
                case .functionSignatureSpecializationParam:
                ...
                case .functionSignatureSpecializationParamPayload:
                    target.write((try? EXBacktrace.parseMangledSwiftSymbol(name.text ?? "").description) ?? (name.text ?? ""))
                case .functionSignatureSpecializationParamKind:
                ...
                case .lazyProtocolWitnessTableAccessor:
                    _ = printOptional(name.children.ex_backtrace_at(0), prefix: "lazy protocol witness table accessor for type ")
                    _ = printOptional(name.children.ex_backtrace_at(1), prefix: " and conformance ")
                case .lazyProtocolWitnessTableCacheVariable:
                    _ = printOptional(name.children.ex_backtrace_at(0), prefix: "lazy protocol witness table cache variable for type ")
                    _ = printOptional(name.children.ex_backtrace_at(1), prefix: " and conformance ")
                ...
                case .resilientProtocolWitnessTable:
                    ...
                    printFirstChild(name)
                case .vTableThunk:
                    _ = printOptional(name.children.ex_backtrace_at(1), prefix: "vtable thunk for ")
                    _ = printOptional(name.children.ex_backtrace_at(0), prefix: " dispatching to ")
                case .protocolWitness:
                    _ = printOptional(name.children.ex_backtrace_at(1), prefix: "protocol witness for ")
                    _ = printOptional(name.children.ex_backtrace_at(0), prefix: " in conformance ")
                case .partialApplyForwarder:
                ...
                case .keyPathGetterThunkHelper:
                    ...
                    _ = printOptional(name.children.ex_backtrace_at(1))
                    _ = printOptional(name.children.ex_backtrace_at(2))
                case .keyPathSetterThunkHelper:
                ...
                case .fieldOffset:
                    printFirstChild(name)
                    _ = printOptional(name.children.ex_backtrace_at(1), prefix: "field offset for ", asPrefixContext: true)
                case .enumCase:
                ...
                case .reabstractionThunkHelper:
                    if options.contains(.shortenThunk) {
                        _ = printOptional(name.children.ex_backtrace_at(name.children.count - 2), prefix: "thunk for ")
                        break
                    }
                    ...
                    _ = printOptional(name.children.ex_backtrace_at(name.children.count - 2), prefix: "from ")
                    _ = printOptional(name.children.ex_backtrace_at(name.children.count - 1), prefix: " to ")
                ...
                case .associatedTypeMetadataAccessor:
                    _ = printOptional(name.children.ex_backtrace_at(1), prefix: "associated type metadata accessor for ")
                    _ = printOptional(name.children.ex_backtrace_at(0), prefix: " in ")
                case .associatedTypeWitnessTableAccessor:
                    _ = printOptional(name.children.ex_backtrace_at(1), prefix: "associated type witness table accessor for ")
                    _ = printOptional(name.children.ex_backtrace_at(2), prefix: " : ")
                    _ = printOptional(name.children.ex_backtrace_at(0), prefix: " in ")
                ...
                case .metatype:
                    ...
                    guard let type = name.children.ex_backtrace_at(name.children.count == 2 ? 1 : 0)?.children.first else { return nil }
                ...
                case .existentialMetatype:
                    ...
                    _ = printOptional(name.children.ex_backtrace_at(name.children.count == 2 ? 1 : 0), suffix: ".Type")
                case .metatypeRepresentation: target.write(name.text ?? "")
                case .associatedTypeRef:
                    printFirstChild(name)
                    target.write(".\(name.children.ex_backtrace_at(1)?.text ?? "")")
                case .protocolList:
                ...
                case .protocolListWithClass:
                    ...
                    _ = printOptional(name.children.ex_backtrace_at(1), suffix: " & ")
                ...
                case .protocolConformance:
                    if name.children.count == 4 {
                        _ = printOptional(name.children.ex_backtrace_at(2), prefix: "property behavior storage of ")
                        _ = printOptional(name.children.ex_backtrace_at(0), prefix: " in ")
                        _ = printOptional(name.children.ex_backtrace_at(1), prefix: " : ")
                    } else {
                        printFirstChild(name)
                        if options.contains(.displayProtocolConformances) {
                            _ = printOptional(name.children.ex_backtrace_at(1), prefix: " : ")
                            _ = printOptional(name.children.ex_backtrace_at(2), prefix: " in ")
                        }
                    }
                ...
                case .dependentGenericSignature:
                    ...
                    for (depth, c) in name.children.enumerated() {
                        ...
                        let count = name.children.ex_backtrace_at(depth)?.index ?? 0
                        for index in 0..<count {
                            target.write(index != 0 ? ", " : "")
                            target.write(EXBacktrace.archetypeName(UInt32(index), UInt32(depth)))
                        }
                    }
                    
                    if lastDepth != name.children.count - 1 {
                        if options.contains(.displayWhereClauses) {
                            printSequence(name.children.ex_backtrace_slice(lastDepth + 1, name.children.endIndex), prefix: " where ", separator: ", ")
                        }
                    }
                    target.write(">")
                ...
                case .dependentGenericConformanceRequirement:
                    ...
                    _ = printOptional(name.children.ex_backtrace_at(1), prefix: ": ")
                case .dependentGenericLayoutRequirement:
                    guard let layout = name.children.ex_backtrace_at(1), let c = layout.text?.unicodeScalars.first else { return nil }
                    printFirstChild(name, suffix: ": ")
                    switch c {
                    ...
                    if name.children.count > 2 {
                        _ = printOptional(name.children.ex_backtrace_at(2), prefix: "(")
                        _ = printOptional(name.children.ex_backtrace_at(3), prefix: ", ")
                        target.write(")")
                    }
                case .dependentGenericSameTypeRequirement:
                    printFirstChild(name)
                ...
                case .dependentGenericType:
                    guard let depType = name.children.ex_backtrace_at(1) else { return nil }
                    printFirstChild(name)
                ...
                case .silBoxTypeWithLayout:
                    ...
                    _ = printOptional(name.children.ex_backtrace_at(1), suffix: " ")
                    _ = printName(layout)
                    if let genericArgs = name.children.ex_backtrace_at(2) {
        ...
        mutating func printEntityType(name: SwiftSymbol, type: SwiftSymbol) {
            if let labelList = name.children.first(where: { $0.kind == .labelList }) {
                ...
                if type.kind == .dependentGenericType {
                    ...
                    if let dt = type.children.ex_backtrace_at(1) {
        ...
        mutating func printEntity(_ name: SwiftSymbol, asPrefixContext: Bool, typePrinting: TypePrinting, hasName: Bool, extraName: String? = nil, extraIndex: UInt32? = nil, overwriteName: String? = nil) -> SwiftSymbol? {
            let multiWordName = extraName?.contains(" ") == true || (hasName && name.children.ex_backtrace_at(1)?.kind == .localDeclName)
            ...
            if hasName || overwriteName != nil {
                ...
                if let o = overwriteName {
                    ...
                    if let one = name.children.ex_backtrace_at(1) {
            ...
            if typePrinting != .noType {
                ...
                if type.kind != .type {
                    guard let nextType = name.children.ex_backtrace_at(2) else { return nil }
                    type = nextType
                }
                ...
                if typePr == .functionStyle {
                    var t = type
                    while t.kind == .dependentGenericType, let next = t.children.ex_backtrace_at(1)?.children.ex_backtrace_at(0) {
                        t = next
                    }
                    switch t.kind {
        ...
        func shouldPrintContext(_ name: SwiftSymbol) -> Bool {
            ...
            if name.kind == .module && name.text?.starts(with: SwiftSymbol.lldbExpressionsModuleNamePrefix) == true {
        ...
        mutating func printFunctionSigSpecializationParam(_ name: SwiftSymbol, index: Int) -> Int {
            guard let firstChild = name.children.ex_backtrace_at(index), let v = firstChild.index else { return index + 1 }
            switch v {
                case FunctionSigSpecializationParamKind.boxToValue.rawValue, FunctionSigSpecializationParamKind.boxToStack.rawValue:
                    _ = printOptional(name.children.ex_backtrace_at(index))
                    return index + 1
                ...
                case FunctionSigSpecializationParamKind.constantPropGlobal.rawValue:
                    ...
                    _ = printOptional(name.children.ex_backtrace_at(index))
                    target.write(" : ")
                    guard let t = name.children.ex_backtrace_at(index + 1)?.text else { return index + 1 }
                    let demangedName = (try? EXBacktrace.parseMangledSwiftSymbol(t))?.description ?? ""
                ...
                case FunctionSigSpecializationParamKind.constantPropFloat.rawValue:
                    ...
                    _ = printOptional(name.children.ex_backtrace_at(index))
                    target.write(" : ")
                    _ = printOptional(name.children.ex_backtrace_at(index + 1))
                    target.write("]")
                    return index + 2
                case FunctionSigSpecializationParamKind.constantPropString.rawValue:
                    ...
                    _ = printOptional(name.children.ex_backtrace_at(index + 1))
                    target.write("'")
                    _ = printOptional(name.children.ex_backtrace_at(index + 2))
                    target.write("'")
                    target.write("]")
                    return index + 3
                case FunctionSigSpecializationParamKind.closureProp.rawValue:
                    ...
                    var idx = index + 2
                    while idx < name.children.count, let c = name.children.ex_backtrace_at(idx), c.kind == .type {
                        _ = printName(c)
                        idx += 1
                        if idx < name.children.count && name.children.ex_backtrace_at(idx)?.text != nil {
                            target.write(", ")
                        }
                    }
                    target.write("]")
        ...
        mutating func printFunctionParameters(labelList: SwiftSymbol?, parameterType: SwiftSymbol, showTypes: Bool) {
            ...
            for tuple in parameters.children.enumerated() {
                if let label = labelList?.children.ex_backtrace_at(tuple.offset) {
        ...
        mutating func printFunctionType(labelList: SwiftSymbol? = nil, _ name: SwiftSymbol) {
            ...
            guard let parameterType = name.children.ex_backtrace_at(startIndex) else { return }
            ...
            _ = printOptional(name.children.ex_backtrace_at(startIndex + 1))
        }
        
        mutating func printBoundGenericNoSugar(_ name: SwiftSymbol) {
            guard let typeList = name.children.ex_backtrace_at(1) else { return }
            printFirstChild(name)
            printChildren(typeList, prefix: "<", suffix: ">", separator: ", ")
        }
        
        func findSugar(_ name: SwiftSymbol) -> SugarType {
            guard let firstChild = name.children.ex_backtrace_at(0) else { return .none }
            ...
            guard let secondChild = name.children.ex_backtrace_at(1) else { return .none }
            ...
            let c0 = unboundType.children.ex_backtrace_at(0)
            let c1 = unboundType.children.ex_backtrace_at(1)
        ...
        mutating func printBoundGeneric(_ name: SwiftSymbol) {
            ...
            switch sugarType {
                case .optional, .implicitlyUnwrappedOptional:
                    if let type = name.children.ex_backtrace_at(1)?.children.ex_backtrace_at(0) {
                        let needParens = !type.kind.isSimpleType
                        _ = printOptional(type, prefix: needParens ? "(" : "", suffix: needParens ? ")" : "")
                        target.write(sugarType == .optional ? "?" : "!")
                    }
                case .array, .dictionary:
                    _ = printOptional(name.children.ex_backtrace_at(1)?.children.ex_backtrace_at(0), prefix: "[")
                    if sugarType == .dictionary {
                        _ = printOptional(name.children.ex_backtrace_at(1)?.children.ex_backtrace_at(1), prefix: " : ")
                    }
                    target.write("]")
...
extension EXBacktrace {
    ...
    /// NOTE: This struct is to avoid clashing with CwlUtils (from which it is taken). If you want to use this struct outside this file, consider including CwlUtils.
    ///
    ...
    static func decodeSwiftPunycode(_ value: String) -> String {
    ...
    static func archetypeName(_ index: UInt32, _ depth: UInt32) -> String {
        var result = ""
        var i = index
        repeat {
...
extension UnicodeScalar {
    ...
    var ex_backtrace_isDigit: Bool { ("0"..."9").contains(self) }
    /// Tests if the scalar is a plain ASCII English alphabet lowercase letter
    var ex_backtrace_isLower: Bool { ("a"..."z").contains(self) }
    /// Tests if the scalar is a plain ASCII English alphabet uppercase letter
    var ex_backtrace_isUpper: Bool {  ("A"..."Z").contains(self) }
    /// Tests if the scalar is a plain ASCII English alphabet letter
    var ex_backtrace_isLetter: Bool { ex_backtrace_isLower || ex_backtrace_isUpper }
}

extension Array {
    func ex_backtrace_at(_ index: Int) -> Element? {
        return self.indices.contains(index) ? self[index] : nil
    }
    
    func ex_backtrace_slice(_ from: Int, _ to: Int) -> ArraySlice<Element> {
