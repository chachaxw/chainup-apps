...
public extension UIView {
    ...
    public func drawDashLine(color: UIColor = UIColor.ThemeLabel.colorMedium){
        self.drawDashLine(lineLength: 2, lineSpacing: 1, lineColor:color)
    }
    ...
    public func drawDashLine(lineLength: Int ,lineSpacing : Int,lineColor : UIColor){
        ...
        let one = lineLength + lineSpacing
        if w%one != 0 {
            w -= lineLength
        }
    ...
    func findAllTextFields() -> [UITextField] {
        var textFields = [UITextField]()
        if let textField = self as? UITextField {
            textFields.append(textField)
        }
        for subview in subviews {
            textFields += subview.findAllTextFields()
        }
        return textFields
    }
