...
//  Chainup
//
//  Created by zewu wang on2020/8/16.
...
public class DateTools: NSObject {
    ...
    public class func getMillTimeStamp(dateStr: String, dateFormat: String = "yyyy-MM-dd HH:mm:ss") -> String{
        EXLogger.debug(message: "dateStr = \(dateStr)")
        if let date = DateTools.dataFromString(timeString: dateStr, dateFormat: dateFormat){
            let timestamp = DateTools.getMillTimeInterval(date: date)
            EXLogger.debug(message: "timestamp = \(timestamp)")
            return timestamp
        }
        return ""
    }
    ...
    public class func getMillTimeInterval(date: Date? = nil) -> String{
        let date = date ?? Date()
...
//        }
        ...
        let formatter = DateHelper.shared.getDateFormatter(dateFormat: dateFormat)
    ...
    public class func dateToString(_ date : Date, dateFormat : String = "yyyy-MM-dd") -> String{
        let dateFormatter = DateHelper.shared.getDateFormatter(dateFormat: dateFormat)
    ...
    public class func millTimeStampToString(_ timestamp:String,dateFormat: String = "yyyy-MM-dd") -> String{
        let res = timestamp.bigDiv("1000")
        if let time = Double(res){
           let result = DateTools.timeStampToString(time)
           return result
        }
        return ""
    }
...
//        return nil
    ...
    //MARK: -根据后台时间戳返回几分钟前，几小时前，几天前 isPravate -> 0day 0min 0s
    public class func updateTimeToCurrennTime(timeStamp: Double,isMillisecond:Bool = true,endTimeStamp:Double, isPravate: Bool = false) -> String {
        //获取当前的时间戳
        if timeStamp == 0 {
...
public class DateHelper {
    ...
    public var cachesDateFormatter = [String: DateFormatter]()
    
    func getDateFormatter(dateFormat: String) -> DateFormatter {
        if let format = DateHelper.shared.cachesDateFormatter[dateFormat]{
            return format
        }
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = dateFormat
        DateHelper.shared.cachesDateFormatter[dateFormat] = dateFormatter
        return dateFormatter
    }
    

    public func timestampToFormattedString(timestamp: TimeInterval) -> String {
        let date = Date(timeIntervalSince1970: timestamp)
        return formattedStringFromDate(date)
    }

    func formattedStringFromDate(_ date: Date) -> String {
        let format = DateHelper.shared.getDateFormatter(dateFormat: "yyyy-MM-dd HH:mm:ss")
