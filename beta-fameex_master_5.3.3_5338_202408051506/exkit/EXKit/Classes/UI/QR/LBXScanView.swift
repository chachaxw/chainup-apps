...
//  swiftScan
//
//  Copyright ©2020年 xialibing. All rights reserved.
//
...
//            print(NSStringFromCGRect(cropRect))
    ...
    func drawScanRect()
    {
        let XRetangleLeft = ceil(viewStyle.xScanRetangleOffset)
        ...
        //扫码区域Y轴最小坐标
        let YMinRetangle = ceil(self.frame.size.height / 2.0 - sizeRetangle.height/2.0 - viewStyle.centerUpOffset)
        let YMaxRetangle = ceil(YMinRetangle + sizeRetangle.height)
...
//            if readyStr.isEmpty { activityView?.center.x += 50 }
//=======
            activityView?.center =  CGPoint(x: self.center.x, y: YMinRetangle + sizeRetangle.height/2)
//>>>>>>> 1.2.3.1
