...
public class EXCameraAlert{
    public static func popAuthAlert(album: Bool? = false){
        
        var title = EXUIDatasource.shared.camecaErrorTip
        if let album = album, album == true{
            title = EXUIDatasource.shared.albumErrorTip
        }
        let alert = EXCommonAlert()
        alert.configAlert(tipImage: nil, title: title, message: nil, btnLayoutStyle: .horizontal, alertCallBack: { type in
            if type == .sure {
