...
public extension EXTextFieldProtocol where Self:UIView {
    ...
    var attributedTextDidChangeSignal: Observable<NSAttributedString?> { textField.rx.attributedText.asObservable().distinctUntilChanged() }
    ///
//    var didChangeSignal: Observable<Void> { NotificationCenter.default.rx.notification(UITextField.textDidChangeNotification, object: textField).map({_ in }).take(until: textField.rx.deallocating) }
    ///
    var editingDidBeginSignal: Observable<Void> { textField.rx.controlEvent(.editingDidBegin).asObservable() }
    var editingDidEndSignal: Observable<Void> { textField.rx.controlEvent(.editingDidEnd).asObservable() }
}
