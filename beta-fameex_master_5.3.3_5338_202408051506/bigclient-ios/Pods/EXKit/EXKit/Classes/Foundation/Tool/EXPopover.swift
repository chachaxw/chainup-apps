...
//  Popover
//
//  Copyright (c)2020 corin8823. All rights reserved.
//
...
open class EXPopover: UIView {
    ...
    public var blackCoverLayerLongpressAction:((UILongPressGestureRecognizer)->Void)?
    ...
    open func show(_ contentView: UIView, point: CGPoint, inView: UIView) {
        if self.dismissOnBlackOverlayTap || self.showBlackOverlay {
            ...
            self.blackOverlayLongPressGestureRecognizer.isEnabled = blackCoverLayerLongpressAction != nil
        }
    ...
    lazy var blackOverlayLongPressGestureRecognizer: UILongPressGestureRecognizer = {
        let longPress = UILongPressGestureRecognizer()
        longPress.rx.event.filter({$0.state == .began}).subscribe(onNext: {[weak self] longPress in
            self?.blackCoverLayerLongpressAction?(longPress)
        }).disposed(by: disposeBag)
        longPress.minimumPressDuration = 0.6
        blackOverlay.addGestureRecognizer(longPress)
        return longPress
    }()
