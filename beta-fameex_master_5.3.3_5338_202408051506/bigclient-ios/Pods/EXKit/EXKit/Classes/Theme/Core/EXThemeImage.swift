...
extension EXBundle {
    public class func image(named name:String, color:UIColor.Ex.Color = .global,options:[UIImage.Ex.Key:Any?]? = nil) -> UIImage? {
        return UIImage.Ex.named(name, color: color, bundle: resource, options: options)
    }
    public class func svgImage(named name:String, color:UIColor.Ex.Color = .global,options:[UIImage.Ex.Key:Any?]? = nil) -> UIImage? {
        guard !name.isEmpty else { return nil }
        if let image = UIImage.Ex.svg(named: name, color: color, bundle: resource, options: options) { return image }
        if let image = UIImage.Ex.svg(named: name, color: color, bundle: EXKitFiveBundle.resource, options: options) { return image }
        return nil
    }
}

extension UIImage.Ex {
    public static func svg(named name:String,
                           color:UIColor.Ex.Color = .global,
                           bundle:Bundle? = nil,
                           size:CGSize) -> UIImage? {
        return svg(named: name, color: color, bundle: bundle, options: [.size: size])
    }
}

extension EXBundle {
    public class func svgImage(named name:String, color:UIColor.Ex.Color = .global,size:CGSize) -> UIImage? {
        return svgImage(named: name, color: color, options: [.size: size])
    }
}
...
public extension UIImage.Ex {
    ...
    struct UTI : OptionSet, CaseIterable {
        ...
        func named(_ name: String, color:UIColor.Ex.Color, bundle:Bundle, options:[UIImage.Ex.Key:Any?]?) -> UIImage? {
            if let image = cachedImage(for: name, color, bundle, options) { return image }
            ...
            cacheIfNeeded(image: image, name: name, color: color, bundle: bundle, options)
            return image
        }
        private static let cacheKey = UUID().uuidString
        ///
        private func cacheKey(for name:String, _ color:UIColor.Ex.Color, _ bundle:Bundle, _ options:[UIImage.Ex.Key:Any?]?) -> NSString {
            "\(self)_\(name)_\(color)_\(coder?.cacheOptions(for: options) ?? UTI.cacheKey)_\(bundle.bundlePath)" as NSString
        }
        ///
        private func cachedImage(for name:String, _ color:UIColor.Ex.Color, _ bundle:Bundle, _ options:[UIImage.Ex.Key:Any?]?) -> UIImage? {
            Self.cache.object(forKey: cacheKey(for: name, color, bundle, options))
        }
        ///
        private func cacheIfNeeded(image:UIImage?, name:String, color:UIColor.Ex.Color, bundle:Bundle, _ options:[UIImage.Ex.Key:Any?]?) {
            guard let image = image, cacheable(bundle: bundle) else { return }
            Self.cache.setObject(image, forKey: cacheKey(for: name, color, bundle, options))
        }
        ///
...
private protocol ExImageCoder {
    ...
    static func cacheOptions(for options:[UIImage.Ex.Key:Any?]?) -> String?
}
...
private extension UIImage.Ex {
    struct SVG : ExImageCoder {
        ...
        ///
        static func cacheOptions(for options: [UIImage.Ex.Key : Any?]?) -> String? {
            let size = (options?[.size] as? CGSize) ?? CGSize(width: -1, height: -1)
            let scale = (options?[.scale] as? CGFloat) ?? UIScreen.main.scale
            let insets = (options?[.insets] as? UIEdgeInsets) ?? .zero
            return "\(size)_\(scale)_\(insets)"
        }
    }
}
...
private extension UIImage.Ex {
    struct System : ExImageCoder {
        ...
        static func cacheOptions(for options: [UIImage.Ex.Key : Any?]?) -> String? { nil }
    }
}
