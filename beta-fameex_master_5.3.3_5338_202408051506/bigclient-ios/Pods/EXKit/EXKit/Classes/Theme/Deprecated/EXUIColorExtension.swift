...
public extension UIColor {
    ...
    class func extRGBA(red : CGFloat , green : CGFloat , blue : CGFloat , alpha : CGFloat)-> UIColor{
        return UIColor.init(red: red/255.0, green: green/255.0, blue: blue/255.0, alpha: alpha)
    }
    
    class func extColorWithHex(_ hex : String, alpha:CGFloat = 1) -> UIColor{
        var hexColor = hex
        ...
        if hexColor.count < 6 { return .white }
    ...
    //将16进制的颜色转化为 rgb
    class func hexColorToRGB(hex: String) -> (CGFloat,CGFloat,CGFloat){
        let hexadecimal = hex
        var cstr = hexadecimal.trimmingCharacters(in:  CharacterSet.whitespacesAndNewlines).uppercased() as NSString;
        let clear:(CGFloat,CGFloat,CGFloat) = (0,0,0)
        if(cstr.length < 6){
            return clear
        }
