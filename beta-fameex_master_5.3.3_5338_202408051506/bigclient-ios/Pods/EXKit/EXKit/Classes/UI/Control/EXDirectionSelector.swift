...
open class EXDirectionSelector: UIControl {
    ...
    public var iconImage: UIImage? {
        ...
        set {
            icon.image = newValue
            if let image = icon.image { iconSize = image.size }
        }
    }
    ...
    public lazy var icon:UIImageView = {
        ...
        icon.image = EXKitBundle.image(named: "public_arrow_down")?.reSizeImage(reSize: .init(width: 12, height: 12))
        return icon
    }()
    ...
    open func config() {
        ...
        self.extSetBorderWidth(1/UIScreen.main.scale, color: .clear)
    ...
    @discardableResult
    public func toggle(animated:Bool = true) -> Bool {
        UIView.animate(withDuration: animated ? 0.2 : 0) {
            self.isOn.toggle()
        }
        return isOn
