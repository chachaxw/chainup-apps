...
//        let profit = position.unrealised_profit.bigDiv(position.im) ?? "0"
        ...
        shareV.swapName.text = position.ex_contractInfo?.showName()
...
class EXShareView: UIView {
    ...
    let qrView: UIImageView = {
        let qrIcon = LBXScanWrapper.createCode(codeType: "CIQRCodeGenerator", codeString: EXSwapPlatformSDK.shared.inviteUrl ?? "", size: CGSize(width: 50, height: 50), qrColor: UIColor.ThemeLabel.colorLite, bkColor: UIColor.ThemeView.bg)
