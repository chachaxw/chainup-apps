...
String long2date(dynamic longDate) {
  if(longDate==null){
    return "";
  }
  return dateFormat(DateTime.fromMillisecondsSinceEpoch(longDate ?? 0),
      [yyyy, '-', mm, '-', dd, ' ', HH, ':', nn, ':', ss]);
}

String long2dateYmd(dynamic longDate) {
  if(longDate==null){
    return "";
  }
  return dateFormat(DateTime.fromMillisecondsSinceEpoch(longDate ?? 0),
...
String long2dateMdhns(dynamic longDate) {
  if(longDate==null){
    return "";
  }
  return dateFormat(DateTime.fromMillisecondsSinceEpoch(longDate ?? 0),
...
String long2datehns(dynamic longDate) {
  if(longDate==null){
    return "--";
  }
  return dateFormat(DateTime.fromMillisecondsSinceEpoch(longDate ?? 0),
...
String long2MD(dynamic longDate) {
  return dateFormat(
      DateTime.fromMillisecondsSinceEpoch(longDate ?? 0), [mm, '-', dd]);
}
...
/// 秒转分秒
String second2MS(int sec, {bool isEasy = false}) {
  String ms = "00:00";
  if (!isEasy) ms = "00'00''";
  if (sec > 0) {
    int m = (sec % 3600) ~/ 60;
    ...
    if (!isEasy) ms = "${zeroFill(m)}'${zeroFill(s)}''";
  }
  return ms;
}
/// 秒转时分秒
...
List<String> second2ListStr(int sec) {
  List<String> list =  ["00", "00", "00", "00"];
  if (sec > 0) {
...
List<int> second2List(int sec) {
  // List<int> list = List(4);
  List<int> list =  [0, 0, 0, 0];
  if (sec > 0) {
    list[0] = sec ~/ 86400; //日
