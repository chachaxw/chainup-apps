...
class ExTextHighlight extends StatelessWidget {
  final TextStyle _ordinaryStyle; // 普通的样式
  final TextStyle _highlightStyle; // 高亮的样式
  final String _content; // 文本内容
  final List<String> _searchContents; // 搜索的内容列表

  const ExTextHighlight(
      this._content,
  ...
  Widget build(BuildContext context) {
    ...
    for (String searchContent in _searchContents) {
      int end;
      // 遍历，进行多处高亮
      while ((end = _content.indexOf(searchContent, start)) != -1) {
