...
public class EXStackView: UIStackView {
    ...
    public override func layoutSubviews() {
        ...
        if width > 0 && width < Separator.Configuration.pixelOfOne { width = 0 }
        if width <= 0 { width = axis == .vertical ? bounds.width : Separator.Configuration.pixelOfOne }
        //
        var height = separatorConfiguration.height
        if height > 0 && height < Separator.Configuration.pixelOfOne { height = 0 }
        if height <= 0 { height = axis == .vertical ? Separator.Configuration.pixelOfOne : bounds.height }
        //
