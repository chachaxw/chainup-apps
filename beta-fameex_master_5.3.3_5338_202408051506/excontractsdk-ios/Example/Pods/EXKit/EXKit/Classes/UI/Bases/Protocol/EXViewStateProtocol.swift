...
public extension EXHighlightable where Self:UIView {
    var isHighlighted: Bool {
        get {
            guard let value = objc_getAssociatedObject(self, &EXViewStateConstant.highlightable.state) as? Bool else { return false }
            return value
        }
        set {
    ...
    var highlightableUpdater:EXViewStateUpdater? {
        get {
            guard let value = objc_getAssociatedObject(self, &EXViewStateConstant.highlightable.updater) as? EXViewStateUpdater else {
                return nil
            }
            return value
        }
...
public extension EXSelectable where Self:UIView {
    var isSelected: Bool {
        get {
            guard let value = objc_getAssociatedObject(self, &EXViewStateConstant.selectable.state) as? Bool else { return false }
            return value
        }
        set {
    ...
    var selectableUpdater:EXViewStateUpdater? {
        get {
            guard let value = objc_getAssociatedObject(self, &EXViewStateConstant.selectable.updater) as? EXViewStateUpdater else {
                return nil
            }
            return value
        }
...
public class EXViewStateUpdater: NSObject {
    ...
    static var `default`: EXViewStateUpdater {
...
public class EXViewStateConfigurationItem: NSObject {
    ...
    public class func borderWidth(off:Any? = 0, on: Any? = 0.5) -> EXViewStateConfigurationItem {
