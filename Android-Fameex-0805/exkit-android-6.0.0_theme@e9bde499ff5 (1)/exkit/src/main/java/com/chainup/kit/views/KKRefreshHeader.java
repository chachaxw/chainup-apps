...
/**
 *Classic Dropdown Head
 * Created by scwang on 2017/5/28.
 */
@SuppressWarnings({"unused", "UnusedReturnValue"})
public class KKRefreshHeader extends KKRefreshAbstract<KKRefreshHeader> implements RefreshHeader {
...
//    public static String REFRESH_HEADER_UPDATE = "'Last update' M-d HH:mm";
    ...
    public KKRefreshHeader(Context context, AttributeSet attrs) {
        super(context, attrs, 0);

        View.inflate(context, com.example.chainup_kit.R.layout.kk_refresh_header, this);

        final View thisView = this;
        final View arrowView = mArrowView = thisView.findViewById(R.id.srl_classics_arrow);
        final View okView = mOkView = thisView.findViewById(R.id.srl_classics_ok);
...
//        }
        ...
        if (ta.hasValue(R.styleable.ClassicsHeader_srlTextSizeTitle)) {
            mTitleText.setTextSize(TypedValue.COMPLEX_UNIT_PX, ta.getDimensionPixelSize(R.styleable.ClassicsHeader_srlTextSizeTitle, SmartUtil.dp2px(12)));
//        } else {
...
//        progressView.animate().setInterpolator(null);
        ...
        (  (LottieAnimationView) progressView).addLottieOnCompositionLoadedListener(new LottieOnCompositionLoadedListener() {
            @Override
            public void onCompositionLoaded(LottieComposition composition) {
                ...
                ( (LottieAnimationView) progressView).addValueCallback(new KeyPath("**"), LottieProperty.COLOR_FILTER, colorCallback);
            }
        });

        try {//Try cannot be deleted - otherwise compatibility issues may occur
    ...
    public int onFinish(@NonNull RefreshLayout layout, boolean success) {
        ...
        mOkView.setVisibility(VISIBLE);

        return super.onFinish(layout, success);//Rebound after a delay of 500 milliseconds
    }
    ...
    public void onStateChanged(@NonNull RefreshLayout refreshLayout, @NonNull RefreshState oldState, @NonNull RefreshState newState) {
        ...
        final View okView = mOkView;
        switch (newState) {
            case None:
                okView.setVisibility(GONE);
                updateView.setVisibility(mEnableLastTime ? VISIBLE : GONE);
    ...
    public KKRefreshHeader setLastUpdateTime(Date time) {
        final View thisView = this;
        mLastTime = time;
    ...
    public KKRefreshHeader setTimeFormat(DateFormat format) {
    ...
    public KKRefreshHeader setLastUpdateText(CharSequence text) {
        mLastTime = null;
        mLastUpdateText.setText(text);
        return this;
    }

    public KKRefreshHeader setAccentColor(@ColorInt int accentColor) {
        mLastUpdateText.setTextColor(accentColor&0x00ffffff|0xcc000000);
        return super.setAccentColor(accentColor);
    }

    public KKRefreshHeader setEnableLastTime(boolean enable) {
        final View updateView = mLastUpdateText;
    ...
    public KKRefreshHeader setTextSizeTime(float size) {
    ...
    public KKRefreshHeader setTextSizeTime(int unit, float size) {
    ...
    public KKRefreshHeader setTextTimeMarginTop(float dp) {
        final View updateView = mLastUpdateText;
    ...
    public KKRefreshHeader setTextTimeMarginTopPx(int px) {
...
//    /**
//     *@ deprecated uses findViewById (ID_TEXTUPDATE) instead
//     */
//    @Deprecated
