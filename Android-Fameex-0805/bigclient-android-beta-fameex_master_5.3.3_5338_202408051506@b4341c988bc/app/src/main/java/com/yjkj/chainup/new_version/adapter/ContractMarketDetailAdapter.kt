...
class ContractMarketDetailAdapter : BaseQuickAdapter<JSONObject, BaseViewHolder>(R.layout.item_market_detail_new) {


    val TAG = ContractMarketDetailAdapter::class.java.simpleName
    ...
    override fun convert(helper: BaseViewHolder, item: JSONObject) {
        ...
        name= CpClLogicContractSetting.getContractShowNameById(context, item.optInt("id"))
...
//        var price = CpClLogicContractSetting.getContractMultiplierPrecisionById(context,item.optInt("id"))
        var rose = item.optString("rose", "")
        var rateResult = item.optString("rateResult")
        try {
            rateResult = RateManager.getContractCNYByContractType(item.optString("quote"), close,name)
        } catch (e: Exception) {
            LogUtil.e("22222",e.message.toString())
            e.printStackTrace()
        }
        val isCollect=  CpClLogicContractSetting.hasCollect(context,item.optInt("id"))
        if (!isMarketLike){
            helper.setGone(R.id.img_market_collect,!isCollect)
        }else{
        ...
        if (TextUtils.isEmpty(vol)) {
            ...
            var multiplier = item.optString("multiplier")
            var multiplierCoin = item.optString("multiplierCoin")
           val mMultiplierPrecision= CpClLogicContractSetting.getContractMultiplierPrecisionByMultiplier(multiplier)
            val amount =  CpBigDecimalUtils.mulStr(
                vol,
                multiplier,
                mMultiplierPrecision
            )
            helper?.setText(R.id.tv_volume, LanguageUtil.getString(context, "common_text_dayVolume") + " " + BigDecimalUtils.showDepthVolume(amount)+" " +multiplierCoin)
        }
