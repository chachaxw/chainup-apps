...
import com.chainup.contract.view.dialog.list.CpTListDialog
...
class DialogUtil {
    ...
    companion object {
        ...
        fun showBottomDialog(context: Context, titles: List<String>, listener: ConfirmListener) {
            CpTListDialog.Builder((context as AppCompatActivity).supportFragmentManager)
                    .setLayoutRes(R.layout.dialog_remove_black_otc)
                    .setScreenWidthAspect(context, 1.0f)
                    .setGravity(Gravity.CENTER)
                    .setDimAmount(0.8f)
                    .setCancelOutside(true)
                    .setAdapter(object : CpTBaseAdapter<String>(R.layout.dialog_bottom_menu, titles) {
                        override fun onBind(holder: CpBindViewHolder?, position: Int, t: String?) {
        ...
        /**iv_cancel
         *Dialogue for fingerprint recognition and facial recognition
         */
        fun showFingerprintOrFaceIDDialog(context: Context, title: String = "", tips: String = "",onCancel: CancelListener?=null):  CpTDialog {
         ...
         *Select lever
         */
        fun showSelectLevelDialog(context: Context, contractId: Int, curLever: String, listener: ConfirmListener):  CpTDialog {
        ...
        /**
         *PopupWindow for spot interface
         */
        fun createCVCPop(context: Context?, targetView: View, fragment: NCVCTradeFragment,mOnMoreCtrlListener:OnMoreCtrlListener) {
            val view = LayoutInflater.from(context).inflate(R.layout.popwindow_cvc_entrance,null)
            ...
            view?.run {
                ...
                if (isHorizontalDepth) {
                    findViewById<ImageView>(R.id.iv_change_direction)?.setImageResource(R.mipmap.exchange_horizontalversion)
                    findViewById<TextView>(R.id.tv_change_direction).text = LanguageUtil.getString(context, "coin_text_horizontalDish")
                } else {
                    findViewById<ImageView>(R.id.iv_change_direction)?.setImageResource(R.mipmap.exchange_verticalversion)
            ...
            val cvcEasyPopup = EasyPopup.create().setContentView(view)
                    .setFocusAndOutsideEnable(true)
                    .setBackgroundDimEnable(true)
                    .setWidth(view.measuredWidth)
            ...
            cvcEasyPopup?.run{
                ...
                findViewById<View>(R.id.ll_recharge)?.setOnClickListener {
                    ...
                    mOnMoreCtrlListener?.onRecharge()
                }
                val llTransfer = findViewById<View>(R.id.ll_transfer)
...
//                        v_horizontal_depth?.refreshDepthView()
        ...
        /**
         *PopupWindow for Grid Interface
         */
        ...
        fun createGridPop(context: Context?, targetView: View, fragment: NGridFragment) {
            val view = LayoutInflater.from(context).inflate(R.layout.popwindow_cvc_entrance,null)
            view.findViewById<View>(R.id.ll_change_pan).visibility = View.GONE
            val intw = View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED)
            val inth = View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED)
            view.measure(intw, inth)
        ...
        /**
         *PopupWindow for lever interface
         */
        fun createLeverPop(context: Context?, targetView: View, fragment: NLeverFragment, symbol: String = PublicInfoDataService.getInstance().currentSymbol4Lever) {
            val view = LayoutInflater.from(context).inflate(R.layout.popwindow_lever_entrance,null)
            val intw = View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED)
            val inth = View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED)
            ...
            view.measure(intw, inth)
            val leverEasyPopup = EasyPopup.create().setContentView(view)
                    .setFocusAndOutsideEnable(true)
                    .setBackgroundDimEnable(true)
                    .setWidth(view.measuredWidth)
                    .setHeight(ViewGroup.LayoutParams.WRAP_CONTENT)
                                ...
                                HttpClient.instance
                                        ...
                                        .subscribe(object : NetObserver<Any>() {
                                            override fun onHandleSuccess(bean: Any?) {
                                                tDialog.dismiss()
//ToastUtils. showToast ("Authorization successful, skipping")
                                                context.finish()
                                            }
        ...
        fun showKLineShareDialog(context: Context) {
            ...
            screenshotBitmap = ScreenShotUtil.getScreenshotBitmap((context as AppCompatActivity).window?.decorView
                    ?: return)
            var ll_share: View? = null
              CpTDialog.Builder(context.supportFragmentManager)
        ...
        fun createCVCOrderPop(context: Context?, index: Int = 0, targetView: View, dialogItemClickListener: NewDialogUtils.DialogOnSigningItemClickListener?) {
            ...
            cvcEasyPopup?.run {
                ...
                val tv_text_recharge = findViewById<TextView>(R.id.tv_text_recharge)
                tv_text_recharge.setText(LanguageUtil.getString(context,"contract_action_limitPrice"))
                val tv_action_transfer = findViewById<TextView>(R.id.tv_action_transfer)
                tv_action_transfer.setText(LanguageUtil.getString(context,"contract_action_marketPrice"))

                val bottomView = findViewById<View>(R.id.ll_transfer)
                if (index == 0) {
                    findViewById<TextView>(R.id.tv_text_recharge).textColor = ColorUtil.getColor(context!!, R.color.main_color)
                    topView.backgroundResource = R.drawable.bg_layout_more_top
                } else {
                    bottomView.backgroundResource = R.drawable.bg_layout_more_top
                    findViewById<TextView>(R.id.tv_action_transfer).textColor = ColorUtil.getColor(context!!, R.color.main_color)
                }
        ...
        fun showGridStatement(context: Context?) {
            ...
            CpTDialog.Builder((context as AppCompatActivity).supportFragmentManager)
                    ...
                    .setCancelableOutside(false)
                    .setOnBindViewListener { viewHolder: CpBindViewHolder? ->
                        viewHolder?.setText(R.id.tv_head,"quant_grid_guide".tr(context))
                        setupOne = viewHolder?.getView(R.id.imageOne)
                        setupTwo = viewHolder?.getView(R.id.imageTwo)
                        setupThree = viewHolder?.getView(R.id.imageThree)
                        var btn_know = viewHolder?.getView<KKButtonKit>(R.id.btn_know)
                        var btn_next = viewHolder?.getView<KKButtonKit>(R.id.btn_next)
                        btn_know?.textContent = (LanguageUtil.getString(context,"alert_common_i_understand"))
                        btn_next?.textContent = (LanguageUtil.getString(context,"common_action_next"))
                        val defDrawableOne = ContextCompat.getDrawable(context,Constants.getGuideOne())
                        val defDrawableTwo = ContextCompat.getDrawable(context,Constants.getGuideTwo())
                        val defDrawableThree = ContextCompat.getDrawable(context,Constants.getGuideThree())
                        Glide.with(context)
                            .load("greatepolicy".toGridOnlinePNG())
                            .placeholder(defDrawableOne)
                            .error(defDrawableOne)
                            .into(setupOne!!)
                        Glide.with(context)
                            .load("policyrun".toGridOnlinePNG())
                            .placeholder(defDrawableTwo)
                            .error(defDrawableTwo)
                            .into(setupTwo!!)
                        Glide.with(context)
                            .load("policystop".toGridOnlinePNG())
                            .placeholder(defDrawableThree)
                            .error(defDrawableThree)
                            .into(setupThree!!)

                        setupOne?.visibility = View.VISIBLE
                    }
                    .addOnClickListener(R.id.btn_know, R.id.btn_next)
                    .setOnViewClickListener { holder, view, tDialog ->
                        when (view.id) {
                            R.id.btn_know -> {
                                tDialog.dismiss()
                            ...
                            R.id.btn_next -> {
                                ...
                                when (setupCount) {
                                    ...
                                    2 -> {
                                        ...
                                        holder.setGone(R.id.btn_know,false)
                                        holder.getView<KKButtonKit>(R.id.btn_next).textContent = LanguageUtil.getString(context,"alert_common_i_understand")
                                    }
                                    else -> {
                                        PublicInfoDataService.getInstance().saveGridStateDialogStatus(true)
                                        tDialog.dismiss()
        ...
        /**
         *Long press the pop-up box for the market
         *IsShowTop: true Display Top False Do not display Top
         *IsCollect: true display favorites false display cancel favorites
         *DialogItemClickListener: position 1 favorite, cancel favorite 0 top, cancel top
         */
        fun createMarketPop(
            context: Context?,
            targetView: View,
            ...
            dialogItemClickListener: NewDialogUtils.DialogOnSigningItemClickListener?
        ) {
            val leverEasyPopup =
                EasyPopup.create().setContentView(context, R.layout.popwindow_market)
                    .setFocusAndOutsideEnable(true)
                    .setWidth(ViewGroup.LayoutParams.WRAP_CONTENT)
                    .setHeight(ViewGroup.LayoutParams.WRAP_CONTENT)
            ...
            leverEasyPopup?.run {
                val topView = findViewById<TextView>(R.id.tv_top)
                val tvCollect = findViewById<TextView>(R.id.tv_collect)
                val img_line = findViewById<ImageView>(R.id.img_line)
                topView.visibility = if (isShowTop == true) View.VISIBLE else View.GONE
                img_line.visibility = if (isShowTop == true) View.VISIBLE else View.GONE
                tvCollect.visibility = View.VISIBLE
                topView.setText(LanguageUtil.getString(context, "market_edit_like_type_top"))
                tvCollect.setText(
                    if (isCollect == true) LanguageUtil.getString(
                        context,
                        "market_str_2"
                    ) else LanguageUtil.getString(context, "market_str_1")
                )
                topView.setOnClickListener {
                    dialogItemClickListener?.clickItem(0,"")
                    dismiss()
                }
            ...
            val windowPos = SystemUtils.calculatePopWindowPos(targetView,leverEasyPopup.contentView)
        ...
        //Display a dialog prompt for missing permissions (native dialog)
        @JvmStatic
        fun showAlertDialog(context: Context,message: String){
            AlertDialog.Builder(context)
                .setMessage(message)
                .setPositiveButton(LanguageUtil.getString(context,"otc_cancel"),object : DialogInterface.OnClickListener{
                    override fun onClick(p0: DialogInterface?, p1: Int) {
                        p0?.dismiss()
                    }
                })
                .setNegativeButton(LanguageUtil.getString(context,"permission_tosetting_msg"),object : DialogInterface.OnClickListener{
                    override fun onClick(p0: DialogInterface?, p1: Int) {
                        PermissionUtil.toSettingActivity(context)
                    }
                })
                .create()
                .show()
