...
public class CoinMapAdapter extends BaseQuickAdapter<JSONObject, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, JSONObject item) {
        ...
        if (leverStatus) {
            ...
            helper.setGone(R.id.balance, true);
...
//                helper.getView(R.id.ib_add).setVisibility(View.VISIBLE);
            ...
            boolean isAdd = LikeDataService.getInstance().hasCollect(symbol);
            if(isAdd){
                try {
             ...
             */
            if (isAdd) {
                ((ImageView) helper.getView(R.id.ib_add)).setImageResource(R.drawable.ic_public_favorites);
            } else {
            ...
            if (TextUtils.isEmpty(close)) {
                ...
                helper.setText(R.id.tv_close_price, BigDecimalUtils.divForDown(close, item.optInt("price")).toPlainString());
            }
