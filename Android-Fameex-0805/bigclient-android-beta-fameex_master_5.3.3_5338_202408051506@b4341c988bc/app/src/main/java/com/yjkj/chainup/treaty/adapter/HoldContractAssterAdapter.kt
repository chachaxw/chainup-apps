...
open class HoldContractAssterAdapter(data: ArrayList<JSONObject>) : BaseQuickAdapter<JSONObject, BaseViewHolder>(R.layout.item_new_contract_asset_otc, data) {
    ...
    override fun convert(helper: BaseViewHolder, item: JSONObject) {
        ...
        helper?.setText(R.id.tv_coin_name, item?.optString("contractSeries") + " " + Contract2PublicInfoManager.getContractType(context, item?.optInt("contractType"), item?.optString("settleTime")) + " (${item?.optString("leverageLevel")}X)")

        helper?.setText(R.id.tv_normal_title, LanguageUtil.getString(context,"contract_text_realisedPNL"))
        helper?.setText(R.id.tv_lock_title, LanguageUtil.getString(context,"contract_text_unrealisedPNL"))
        helper?.setText(R.id.tv_equivalent, LanguageUtil.getString(context,"contract_text_positionNumber") + "(${LanguageUtil.getString(context,"contract_text_volumeUnit")})")
