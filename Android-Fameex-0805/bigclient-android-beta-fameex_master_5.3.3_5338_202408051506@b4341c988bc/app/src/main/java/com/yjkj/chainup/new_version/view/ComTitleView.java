...
public class ComTitleView extends RelativeLayout {
    ...
    public ComTitleView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        backIcon = typedArray.getResourceId(R.styleable.ComTitleView_backIcon, R.drawable.ic_return);
        ...
        if (ibBack != null) {
            ...
            ibBack.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View view) {
                    if (!isWeb) ((Activity) context).finish();
                }
            });
        }
        ibClose = view.findViewById(R.id.ib_close);
        ibClose.setOnClickListener(v -> ((Activity) context).finish());
    ...
    public void changeClose(boolean isShow) {
        ibClose.setVisibility(isShow ? View.VISIBLE : View.INVISIBLE);
    }

    private boolean isWeb = false;

    public void setWeb(boolean isShow) {
        isWeb = isShow;
    }
