...
/**
 *@description: otc/public_ Information interface data
 * @Author: wanghao
 * @CreateDate: 2019-10-23 15:52
 * @UpdateUser: wanghao
 * @UpdateDate 2023-10-23 15:52
 *@ UpdateRemark: Update Description
 */
public class OTCPublicInfoDataService {

    /*
     *The name of the key should be consistent with the interface name, and should be underlined_ replace
     */
    ...
    public ArrayList<JSONObject> getPaycoins() {
        JSONObject data = getData();
        if (null != data) {
            if(JSONUtil.arrayToList(data.optJSONArray("paycoins"))==null){
    ...
    public JSONObject getOTCPaymentInfoByKey(String key) {
        ArrayList<JSONObject> payments = getPayments();
        for(JSONObject item : payments) {
