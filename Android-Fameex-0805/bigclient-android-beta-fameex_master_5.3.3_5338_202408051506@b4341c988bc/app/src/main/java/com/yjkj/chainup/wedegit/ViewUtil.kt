...
object ViewUtil {
    ...
    fun showSuccesSnackbar(view: View, text: String, showLength: Int): TSnackbar {
        var tSnackbar = TSnackbar.make(view, text, showLength)
        var snackbarView = tSnackbar.view
        snackbarView.setBackgroundColor(ColorUtil.getColor(R.color.base_green))
        val textView = snackbarView.findViewById(com.androidadvance.topsnackbar.R.id.snackbar_text) as TextView
        textView.gravity = Gravity.CENTER
        textView.setTextSize(TypedValue.COMPLEX_UNIT_SP, 16f)
        textView.setTextColor(Color.WHITE)
    ...
    fun getIndicatorOvalDrawable(context: Context, selectColor: Int, unSelectColor: Int, radius: Int = ViewUtil.dpToPx(12f), margin: Int = ViewUtil.dpToPx(8f)): ImageView {
    ...
    /**
     *Set the width of the TabLayout indicator line
     */
    fun reflex(tabLayout: TabLayout) {
        ...
        tabLayout.post {
            try {
                //Obtain the mTabStrip attribute of tabLayout
                ...
                for (i in 0 until mTabStrip.childCount) {
                    ...
                    //Obtaining the mTextView property of tabView with an unfixed word count must use reflection to retrieve mTextView
                    ...
                    //Because the effect I want is to have as many lines as the width of the word, measure the width of mTextView
                    var width = 0
                    ...
                    //Set the left and right spacing of the tab to 10dp. Note that Padding cannot be used here because the width of the source code centerline is set based on the width of the tabView
    ...
    fun setBgColor(view: View, @ColorRes id: Int) {
        view.setBackgroundColor(ColorUtil.getColor(id))
    }

    fun setTxColor(view: TextView, @ColorRes id: Int) {
        view.setTextColor(ColorUtil.getColor(id))
    }



    /**
     *Implement text replication function
     */
    ...
    /**
     *Implement text pasting function
     */
    ...
    fun getFlowManager(context: Context?): FlexboxLayoutManager {
        val gridManager = FlexboxLayoutManager(context)
        gridManager.apply {
