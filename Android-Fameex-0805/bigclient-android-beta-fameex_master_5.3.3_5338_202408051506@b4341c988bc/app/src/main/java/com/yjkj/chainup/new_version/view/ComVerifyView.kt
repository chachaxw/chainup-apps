...
import kotlinx.android.synthetic.main.item_textview_edittext.view.cet_view
import org.jetbrains.anko.runOnUiThread
import java.util.concurrent.TimeUnit

/**
 * @Author: Bertking
 * @Date 2023/3/7-2:20 PM
 *@description: Mobile&Email&Google Verified Common Components
 */
class ComVerifyView @JvmOverloads constructor(context: Context,
                                              attrs: AttributeSet? = null,
                                              defStyleAttr: Int = 0
) : LinearLayout(context, attrs, defStyleAttr),IEditTextUiError {
    ...
    init {
        attrs?.let {
            ...
            mTextHint = typedArray.getString(R.styleable.ComVerifyView_textHint)
            typedArray.recycle()
        }
        /**
        ...
        tv_code?.text = LanguageUtil.getString(context, "otc_action_sendmsg")
    ...
    fun setType(type: Int) {
        ...
        tv_code?.text = if (type == GOOGLE) {
            ...
            LanguageUtil.getString(context, "otc_action_sendmsg")
        }
    ...
    private fun setHintText(type:Int=verifyType){
        when (type) {
            GOOGLE -> {
            ...
            EMAIL -> {
                setHintEditText(LanguageUtil.getString(context,"email_ver_hint"))
            }
        }
    }
                ...
                .flatMap {
                    ...
                    if (it.isSuccess) {
                        ...
                        context.runOnUiThread {
                            ...
                            //Used to solve logout issues and redirect login
                            if(otypeForPhone == AppConstant.ACCOUNT_DELETE_PHONE && "10002".equals(it.code)){
                                clearLoginStatus()
                            }
                        }
                ...
                .flatMap {
                    ...
                    if (it.isSuccess) {
                        ...
                        context.runOnUiThread {
                            view.isClickable = true
                            //Used to solve logout issues and redirect login
                            if(otypeForEmail == AppConstant.ACCOUNT_DELETE_EMAIL && "10002".equals(it.code)){
                                clearLoginStatus()
                            }
                        }
                        null
    ...
    fun setHintEditText(str: String) {
        et_input_code.hint =str

    }

    fun setMaxInputLen(len: Int) {
        et_input_code.filters = arrayOf<InputFilter>(InputFilter.LengthFilter(len))
    }

    //Clear login status
    private fun clearLoginStatus() {
        EventBusUtil.post(MessageEvent(MessageEvent.destroy_account_event))
    ...
    override fun changeError(message:String) {
        if("".equals(message)){
            tv_error_tip.visibility = GONE
        }else{
            tv_error_tip.visibility = VISIBLE
            tv_error_tip.text = message
        }
        v_line.setBackgroundResource(R.drawable.et_underline_focused_error)
    }

    override fun resetEditTextColor() {
        v_line.setBackgroundResource(if (et_input_code.hasFocus()) R.color.main_blue else R.color.new_edit_line_color)
        tv_error_tip.visibility = GONE
    }
}
