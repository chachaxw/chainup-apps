 ...
 * @author Bertking
 * @Date 2023/5/18
 *@description account interface (public ItemView)
 */
public class AccountItemView extends RelativeLayout {
    ...
    public ImageView iv_red_dot_left;
    ...
    private void initView(Context context) {
        ...
        iv_red_dot_left = view.findViewById(R.id.iv_icon_right);
...
//        tvState.setVisibility(!showArrow ? VISIBLE : GONE);
    ...
    public void showMailRed(boolean status, int res) {
        if (status) {
            iv_red_dot_mail.setVisibility(View.VISIBLE);
            if (res != 0) {
                iv_red_dot_mail.setImageResource(res);
            }
        } else {
            iv_red_dot_mail.setVisibility(View.GONE);
        }
    ...
    public void showLeftRed(boolean status) {
        if (status) {
            iv_red_dot_left.setVisibility(View.VISIBLE);
        } else {
            iv_red_dot_left.setVisibility(View.GONE);
        }

    }

    public void setAction(String value,OnClickListener clickListener){
        tvAction.setVisibility(View.VISIBLE);
        tvAction.setText(value);
