...
class CommonlyUsedButton @JvmOverloads constructor(
    ...
    var clickBgColor = ContextCompat.getColor(context, R.color.main_blue_click)
    ...
    init {
        attrs.let {
            ...
            clickBgColor = typedArray.getColor(R.styleable.CommonlyUsedButtonView_bgClickColor, ContextCompat.getColor(context, R.color.main_blue_click))
    ...
    fun initView(context: Context) {
        ...
        rl_layout.setBackgroundColor(R.drawable.kk_btn_1_bg)
    ...
    /**
     *Can I click on it
     *@param status true can be clicked, false cannot be clicked
     */
    fun isEnable(status: Boolean) {
        if (status) {
            bg_color.backgroundDrawable = getDrawable(true)
            rl_layout.backgroundDrawable = getMainLayoutDrawable(true)
            tv_complainCommand_content.textColor = textColor
        } else {
            bg_color.backgroundDrawable = getDrawable(false)
            rl_layout.backgroundDrawable = getMainLayoutDrawable(true)
            tv_complainCommand_content.textColor = ContextCompat.getColor(context, R.color.text_color_2)
        }
        isEnabled = status
        clicked = status
    }
    ...
    fun getDrawable(isEnable: Boolean): RippleDrawable {
        ...
        val stateColorListNoEnable = intArrayOf(
            noEnableColor,
            noEnableColor,
            noEnableColor,
            noEnableColor
        ...
        val colorStateListNoEnable = ColorStateList(stateList, stateColorListNoEnable)

        val outRadius = floatArrayOf(10f, 10f, 10f, 10f, 10f, 10f, 10f, 10f)
        ...
        //ContentDrawable is actually displayed by default during initialization; MaskDrawable controls the scope of RippleDrawable
        if (isEnable) {
            contentDrawable.paint.color = normalBgColor
            maskDrawable.paint.color = clickBgColor
            rippleDrawable = RippleDrawable(colorStateList, contentDrawable, maskDrawable)
        } else {
            contentDrawable.paint.color = noEnableColor
            rippleDrawable = RippleDrawable(colorStateListNoEnable, contentDrawable, null)
        }
