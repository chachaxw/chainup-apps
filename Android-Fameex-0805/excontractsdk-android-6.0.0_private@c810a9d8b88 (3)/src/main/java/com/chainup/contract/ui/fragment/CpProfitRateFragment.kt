 ...
 *Closing price
 */
class CpProfitRateFragment : CpNBaseFragment(),TextWatcher {
        ...
        btn_calculate.listener = object : CpCommonlyUsedButton.OnBottonListener {
            override fun bottonOnClick() {
                ...
                if (TextUtils.isEmpty(etPositionStr) || CpBigDecimalUtils.compareTo(etPositionStr, "0") != 1) {
                    CpNToastUtil.showTopToastNet(this@CpProfitRateFragment.mActivity,false, CpLanguageUtil.getString(context,"cp_extra_text119"))
                    return
                }
                if (TextUtils.isEmpty(etOpenPriceStr) || CpBigDecimalUtils.compareTo(etOpenPriceStr, "0") != 1) {
                    CpNToastUtil.showTopToastNet(this@CpProfitRateFragment.mActivity,false, CpLanguageUtil.getString(context,"cp_extra_text35"))
                    return
                }
                val closePrice = CpBigDecimalUtils.calcClosePriceValue(mContractJson.optString("contractSide").equals("1"), currDirectionInfo?.index!!, etPositionStr, etOpenPriceStr, selectLeverage.toString(), CpClLogicContractSetting.getContractSymbolPricePrecisionById(mActivity,contractId))
                //When the closing price is calculated to be negative, it indicates that the yield cannot be reached
                if (CpBigDecimalUtils.compareTo(closePrice,"0") != 1) {
                    CpNToastUtil.showTopToastNet(this@CpProfitRateFragment.mActivity,false, CpLanguageUtil.getString(context,"cp_extra_text118"))
                    return
                }

                val tabList = ArrayList<CpTabInfo>()
                tabList.add(CpTabInfo(CpLanguageUtil.getString(context,"cp_calculator_text9"), closePrice + mContractJson.optString("quote")))
...
//        CpSlDialogHelper.showCalculatorResultDialog(mActivity!!, tabList)
    ...
    private fun createDefaultResult(){
        resultContent.visibility = View.VISIBLE
        val tabList = ArrayList<CpTabInfo>()
        tabList.add(CpTabInfo(CpLanguageUtil.getString(context,"cp_calculator_text9"), "--" + mContractJson.optString("quote")))
...
//                    typeface = Typeface.defaultFromStyle(Typeface.BOLD)
    ...
    override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
        ...
        if(etOpenPriceVal.isNotEmpty() && etPositionVal.isNotEmpty() && etInputVal.isNotEmpty()){
            btn_calculate.isEnable(true)
        }else{
