...
class CpKLineBean : CpIKLine {
    ...
    override var closePrice: Float = 0f
        get() {
            ...
            return CpDecimalUtil.cutValueByPrecision(field.toString(), price).toFloat()
        }
    ...
    fun getCloseTempPrice(close: String, price: Int?): Float {
        if (price != null) {
            return CpDecimalUtil.cutValueByPrecision(close, price).toFloat()
        }
        return BigDecimal(close).toFloat()
    }
