 ...
 * @Description:
 */
public class CpLabelRadioButton extends androidx.appcompat.widget.AppCompatRadioButton {

    CpLabelViewHelper utils;

    private Context context;

    public CpLabelRadioButton(Context context) {
        this(context, null);
        this.context = context;
    }

    public CpLabelRadioButton(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
        this.context = context;
    }

    public CpLabelRadioButton(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setBg(Boolean status) {
        if (status) {
            setBackgroundResource(R.drawable.cp_bg_new_trading_select);
        } else {
            setBackgroundResource(R.drawable.cp_bg_new_trading);
        }
