...
import com.yjkj.chainup.new_version.kline.formatter.CpBigValueFormatter
 ...
 * @Description:
 */
class CpVolumeView() : CpIChartViewDraw<CpVolumeBean>, CpIFallRiseColor {
    lateinit var view:CpKLineChartView
    var mPricePrecision:Int = 0
    constructor(vw: CpKLineChartView):this(){
        view = vw
        initHandler()
    ...
    fun setPricePrecision(precision:Int){
    ...
    override fun drawTranslated(lastPoint: CpVolumeBean?, curPoint: CpVolumeBean, lastX: Float, curX: Float, canvas: Canvas, view: CpBaseKLineChartView, position: Int) {
        ...
        /**
         *Draw a histogram of transaction volume
         */
        drawHistogram(canvas, curPoint, lastPoint, curX, view, position)

        /**
         *MA5, MA10 of draw transaction volume
         */
        if (isLine) {
    ...
    override fun drawText(canvas: Canvas, view: CpBaseKLineChartView, position: Int, x: Float, y: Float) {
        ...
        var text = "VOL:" + CpBigDecimalUtils.showDepthAmount(view.context,point.volume.toString(),mPricePrecision) + "     "
        ...
        if (isLine) {
            ...
            text = "MA5:" + CpBigDecimalUtils.showDepthAmount(view.context,point.volume4MA5.toString(),mPricePrecision) + "     "
            canvas.drawText(text, textLen, y, paint4MA5)
            textLen += paint4MA5.measureText(text)
            text = "MA10:" + CpBigDecimalUtils.showDepthAmount(view.context,point.volume4MA10.toString(),mPricePrecision)
    ...
    override fun getValueFormatter(): CpIValueFormatter {
        return CpBigValueFormatter(mPricePrecision)
    }
    ...
    /**
     *Set the color of the MA10 line
     * @param color
     */
    ...
    private fun drawHistogram(
        ...
        if (isLine) {
            if (curPoint.closePrice >= curPoint.openPrice) {//Up
