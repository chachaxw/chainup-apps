 ...
 */
class CpContractCurrentEntrustNewFragment : CpNBaseFragment(),View.OnClickListener {
    ...
    var isCurrentContract:Boolean? = null
        get() {
            return if(field==null){
                val isSelect = CpPreferenceManager.getInstance(mActivity).getSharedBoolean(CpPreferenceManager.PREF_SHOW_CURRENT_CONTRACT_CurrentEntrust, false)
                isSelect
            }else{
                field
            }
    ...
    override fun initView() {
        ...
        adapter = CpContractCurrentEntrustNewAdapter(requireActivity(), mList)
        ...
        adapter?.setOnItemChildClickListener { adapter, view, position ->
            ...
            cancelOrder(item.contractId, item.orderId!!, false)
        }
    ...
    private fun switchCurrentContractOff(){
        ...
        CpPreferenceManager.getInstance(mActivity).putSharedBoolean(CpPreferenceManager.PREF_SHOW_CURRENT_CONTRACT_CurrentEntrust,isCurrentContract!!)
        switch_contract_type.setImageResource(if (isCurrentContract!!) R.drawable.trade_switch_open else R.mipmap.trade_switch_close)
    }
                ...
                CpNewDialogUtils.showDialogNew(
                    ...
                    object : CpNewDialogUtils.DialogBottomListener {
                        override fun sendConfirm() {
                            cancelOrder(if(isCurrentContract==true) mContractId.toString() else "","",false)
                        }
                    },
