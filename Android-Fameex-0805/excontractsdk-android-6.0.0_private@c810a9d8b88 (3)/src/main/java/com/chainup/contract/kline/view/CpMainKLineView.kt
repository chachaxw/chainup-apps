 ...
 *@ Description: Main Map of K Line
 */
class CpMainKLineView(kLineChartView: CpKLineChartView) : CpIChartViewDraw<CpCandleBean>, CpIFallRiseColor {
    ...
    private var mMultiplierPrecision = -1
    ...
    private var isDrawMarker = true
    private var isSmallKline = false
    ...
    init {
        ...
        refreshMarkerBorderPaint()

        markerBorderPaint.style = Paint.Style.STROKE
    }

    fun setDrawMarker(isDraw:Boolean){
        isDrawMarker = isDraw
    }

    fun setSmallKline(isSmall:Boolean){
        isSmallKline = isSmall
        refreshMarkerBorderPaint()
    }

    private fun refreshMarkerBorderPaint(){
        markerBorderPaint.color = if(CpClLogicContractSetting.getThemeMode(context)==0 && isSmallKline){
            Color.parseColor("#EDEFF2")
        }else{
            ContextCompat.getColor(context, R.color.marker_border)
        }
    }
        ...
        if (isLine) {
            if (status == MainKlineViewStatus.MA) {
                if (point!!.price4MA60 != 0f) {
                    val text = "MA60:" + view.formatValueWithPrecision(point.price4MA60,mPricePrecision) + "     "
                    canvas.drawText(text, x + CpDisplayUtil.dip2px(5f), textHeight, paint4MA10)
                }
            } else if (status == MainKlineViewStatus.BOLL) {
                if (point!!.mb != 0f) {
                    val text = "BOLL:" + view.formatValueWithPrecision(point.mb,mPricePrecision) + "     "
            ...
            if (status == MainKlineViewStatus.MA) {
                ...
                if (point!!.price4MA5 != 0f) {
                    text = "MA5:" + view.formatValueWithPrecision(point.price4MA5,mPricePrecision)
                    canvas.drawText(text, textLen + CpDisplayUtil.dip2px(5f), textHeight, paint4MA5)
                    textLen += paint4MA5.measureText(text)
                }
                if (point.price4MA10 != 0f) {
                    text = "MA10:" + view.formatValueWithPrecision(point.price4MA10,mPricePrecision)
                    canvas.drawText(text, textLen + CpDisplayUtil.dip2px(20f), textHeight, paint4MA10)
                    textLen += paint4MA10.measureText(text)
                }
                if (point.price4MA30 != 0f) {
                    text = "MA30:" + view.formatValueWithPrecision(point.price4MA30,mPricePrecision)
                    canvas.drawText(text, textLen + CpDisplayUtil.dip2px(30f), textHeight, paint4MA30)
                }
            } else if (status == MainKlineViewStatus.BOLL) {
                if (point!!.mb != 0f) {
                    var textLen = x
                    var text = "BOLL:" + view.formatValueWithPrecision(point.mb,mPricePrecision) + "     "
                    canvas.drawText(text, textLen + CpDisplayUtil.dip2px(5f), textHeight, paint4MA10)
                    textLen += paint4MA5.measureText(text)
                    text = "UB:" + view.formatValueWithPrecision(point.up,mPricePrecision) + "     "
                    canvas.drawText(text, textLen, textHeight, paint4MA5)
                    textLen += paint4MA10.measureText(text)
                    text = "LB:" + view.formatValueWithPrecision(point.dn,mPricePrecision)
                    canvas.drawText(text, textLen, textHeight, paint4MA30)
                }
            }
        }
        if (view.isLongPress && isDrawMarker) {
    ...
    private fun drawMarker(view: CpBaseKLineChartView, canvas: Canvas) {
        ...
        val left: Float
        val top = if(isSmallKline){
            margin
        }else{
            context.dip(36.0f).toFloat()
        }
        ...
        map[CpLanguageUtil.getString(context, "cp_kline_info1")] = RateManager.getAbsoluteText4Kline(lines)
        //Growth rate
        var fist = CpBigDecimalUtils.div(lines, point.openPrice.toString()).toPlainString()
        map[CpLanguageUtil.getString(context, "cp_kline_info2")] = RateManager.getRoseText4Kline(fist)
        //Volume
        map[CpLanguageUtil.getString(context, "cp_marker_kline_text_volume")] = CpBigDecimalUtils.showDepthVolume(context,CpBigDecimalUtils.showSNormal(point.volume.toString()),mMultiplierPrecision)
        ...
        for ((k, v) in map) {
            if (CpLanguageUtil.getString(context,"cp_kline_info1") == k || CpLanguageUtil.getString(context, "cp_kline_info2") == k) {
                markerValuePaint.color = CpColorUtil.getMainColorType(!v.contains("-"),CpBigDecimalUtils.compareTo(v.replace("%",""),"0")==0)
            } else {
    ...
    fun setMaMultiplierPrecision(precision:Int){
        this.mMultiplierPrecision = precision
    }
    ...
    /**
     *Set the selector title text color (time, on, high, low, close)
     * @param color
     */
    ...
    /**
     *Set selector numeric text color
     * @param color
     */
    ...
    fun setMarkerTextSize(textSize: Float) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            markerValuePaint.typeface = context.resources.getFont(R.font.dinpro_regular)
            markerTitlePaint.typeface = context.resources.getFont(R.font.dinpro_regular)
        }
    ...
    fun setMarkerBackgroundColor(color: Int) {
        Handler(Looper.myLooper()!!).postDelayed(object :Runnable{
            override fun run() {
                markerBgPaint.color = if(CpClLogicContractSetting.getThemeMode(context)==0 && isSmallKline){
