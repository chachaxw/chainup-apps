...
open class CpKlineMoreViceTargetAdapter(data: ArrayList<CpTabInfo>) : BaseQuickAdapter<CpTabInfo, BaseViewHolder>(R.layout.cp_item_kline_ctrl_more, data) {

        override fun convert(helper: BaseViewHolder, item: CpTabInfo) {
            helper?.setText(R.id.tv_content, item?.name)
            if (CpKLineUtil.getViceIndex()==helper.adapterPosition){
