...
object CpDataManager {
    val TAG = CpDataManager::class.java.simpleName

    /**
     *Calculate RSI
     *RSI typically takes a 14 day cycle.
     * https://zhidao.baidu.com/question/141764710.html
     *
     * @param dataList
     */
    fun calculateRSI(dataList: List<CpKLineBean>) {
        var rsi: Float?
        var rsiABSEma = 0f
        var rsiMaxEma = 0f
     ...
     * @param dataList
     */
    fun calculateKDJ(dataList: List<CpKLineBean>) {
        var k = 0f
        var d = 0f
        for (i in dataList.indices) {
            val point = dataList[i]
     ...
     * @param dataList
     */
    internal fun calculateWR(dataList: List<CpKLineBean>) {
        var r: Float?
        for (i in dataList.indices) {
            val point = dataList[i]
            var startIndex = i - 14
     ...
     * @param dataList
     */
    internal fun calculateMACD(dataList: List<CpKLineBean>) {
        var ema12 = 0f
        var ema26 = 0f
        var dif = 0f
        var dea = 0f
...
//            macd = (dif - dea) *2f//pm Let's not need * 2
            macd = (dif - dea)
            point.DIF = dif
            point.DEA = dea
            point.MACD = macd
    ...
    /**
     *The calculation of BOLL needs to be performed after the calculation of ma
     *
     * @param dataList
     */
     ...
     * @param dataList
     */
    fun calculateMA(dataList: List<CpKLineBean>) {
        var ma5 = 0f
        var ma10 = 0f
        var ma20 = 0f
        var ma30 = 0f
    ...
    private fun calculateVolumeMA(entries: List<CpKLineBean>) {
        var volumeMa5 = 0f
        var volumeMa10 = 0f
